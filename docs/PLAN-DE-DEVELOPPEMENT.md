...
| S61 | 3 h | Le micro par fenêtre, **sans transcription**. Sélection du périphérique — jamais l'entrée par défaut, exclusion des pilotes de boucle par fonction PURE sur les marqueurs réels (`MSLoopbackDriver` est sur cette machine), préférence micro interne. une **`AVCaptureSession`** par fenêtre — l'`inputNode` d'`AVAudioEngine` refuse tout périphérique imposé sur macOS 26.1, mesuré en vivant (lot5-seuils § 9) —, converter créé une fois hors du rappel, latence § 3.6 sommée et journalisée, périphérique débranché → reconstruire et remesurer. Fermeture d'office : `IsSecureEventInputEnabled()` et `suspended` | L'autotest de sélection exclut « Microsoft Teams Audio » et préfère l'interne sur la liste réelle de la machine ; la latence est au journal avec ses trois termes ; verrouiller l'écran pendant une fenêtre la ferme d'office avec sa trace. `when.isHostTimeValid` faux → repli compteur d'échantillons, tracé |
| S62 | 3 h | La chaîne `SpeechAnalyzer` **au fil de l'eau** : transcriber **sans** `SpeechDetector` — gardé d'abord en défense en profondeur, puis retiré en S64 quand il a avalé la seconde phrase d'un monologue en conditions réelles (lot5-seuils § 10) —, `AnalyzerInput(bufferStartTime:)` portant le temps de session compensé — en échantillons entiers, jamais en secondes flottantes —, `AssetInventory` sans condition, deux tampons — le final AJOUTE, le volatil REMPLACE | Le harnais de S59 **mesure** les trois modes (nominal, sans detector, plus vite que le réel) et consigne leurs comptes de segments dans `lot5-seuils.md` — sur macOS 26.1 ils sont identiques, les faits de sonde d'ADR-0012 ne se manifestent plus, et le harnais le re-dira à chaque mise à jour système. Concaténer les volatils fait rougir |
...
Et S64, en vivant : **le moteur ne rend rien pendant les douze premières
secondes d'une fenêtre** — la transcription est progressive en temps audio,
pas en temps réel. La prolongation par les volatils et le critère de l'éclair
silencieux reposent donc sur l'énergie des tranches audio, seul signal de
parole qui existe tout de suite (lot5-seuils § 10). Et le `SpeechDetector`
qu'ADR-0012 disait obligatoire **avale la parole** face au bruit d'un vrai
micro : retiré, avec l'interrupteur pour re-vérifier à chaque version système.
