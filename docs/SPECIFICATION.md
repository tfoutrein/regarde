...
**Limite assumée du critère de surface** : les `dirtyRects` rapportent ce que le compositeur considère changé, pas ce qui a changé en pixels. Une application à rendu GPU qui re-soumet sa surface entière — Warp le fait à chaque battement de curseur, mesuré le 23 août 2026 à 0,981 de surface moyenne sur un terminal immobile — passe donc le critère et déclenche le burst, pour trois images identiques. Distinguer une surface re-soumise d'une vraie animation exigerait de comparer les pixels, ce que le § 5.1 exclut par coût (440 MiB/s). Le surcoût est borné : trois images au lieu d'une, uniquement sur les marques posées sur ce type d'application.
