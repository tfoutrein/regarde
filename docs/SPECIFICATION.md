...
**Écrans virtualisés** : un affichage servi par un transport logiciel — dock DisplayLink, recopie AirPlay — re-présente son framebuffer entier à chaque frame, et ses `dirtyRects` valent en permanence un rect unique pleine taille (mesuré le 24 août 2026 : `1 rect (0, 0) 1920×1080` sur 358 frames sur 359, Bureau nu). Sur ces écrans le critère de surface est structurellement aveugle et le burst se déclenche à chaque marque — trois images au lieu d'une, coût borné. Même verdict pour un papier peint vidéo, qui anime le Bureau entier sous les fenêtres. La ligne `dernière frame` du bilan de flux rend ces situations diagnosticables en une lecture.
