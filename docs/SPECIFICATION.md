...
| Encodeur | `AVAssetWriter` + `AVVideoMaxKeyFrameIntervalDurationKey = 0.5` (1,0 avant l'amendement S43) | `SCRecordingOutput` | Aucun contrôle du GOP ; et toute `updateConfiguration` **arrête l'enregistrement** sans avertissement. |
...
>
> **Quatrième amendement : le GOP passe de 1,0 s à 0,5 s.** Depuis que les tolérances sont nulles, le décodeur remonte à l'image clé précédente pour chaque image demandée — le GOP borne directement ce trajet, et il borne aussi le coût total de l'extraction. Mesuré le 23 août 2026 : 3 659 ms pour 30 images à GOP 1 s, contre un budget de 3 s (§ 5.5), et le tri croissant des instants n'y a presque rien changé (3 808 → 3 659). À 0,5 s, le trajet de décodage moyen est divisé par deux. Le prix — deux fois plus d'images clés — reste loin du budget disque du § 4.4.
>
...
### 5.1 bis L'épreuve d'identité (amendement S45)

Le § 5.1 exclut la comparaison intégrale des pixels — 440 MiB/s de bande passante mémoire. Mais la recette du 24 août 2026 a prouvé qu'on ne peut pas non plus croire les `dirtyRects` sur parole : un moniteur 1080p en HDMI direct les tenait à un rect pleine taille sur 643 frames sur 644, pixels immobiles à 0,03 % près (§ 5.4, « écrans aux dégâts fantômes »). Coût mesuré de cette crédulité : 7,95 Mb/s d'encodage d'un Bureau figé, 970 MiB en dix minutes à deux écrans, 5 % de CPU — deux budgets du § 4.4 crevés par du vide.

L'épreuve d'identité tient le milieu : **~2 000 points échantillonnés par frame** — huit kilo-octets lus, sans rapport avec la comparaison intégrale — confrontés à la dernière frame *acceptée*. Tous égaux : la frame est du repos déguisé, elle n'est ni écrite, ni comptée au mouvement, ni offerte à l'anneau — le comportement exact d'un écran honnête, qui n'aurait rien envoyé. Un seul écart : la frame est acceptée et devient la référence.

Les positions **tournent** — un seizième renouvelé par frame, l'ensemble en ~une seconde — parce que des mailles fixes seraient des angles morts permanents. Les positions fraîches ne votent pas à leur première frame : elles s'amorcent, puis votent. Le compromis assumé : un changement de quelques dizaines de pixels peut passer entre les mailles quelques frames durant ; un changement d'une surface qui compte ne le peut pas — la probabilité de tout rater s'effondre exponentiellement avec la surface. Et une frame sautée est un simple trou de PTS, que le second passage du § 5.2 sait déjà servir — par l'image d'avant, dont l'épreuve vient précisément d'établir qu'elle est la même.
...
**Écrans aux dégâts fantômes** : sur certains affichages externes — mesuré le 24 août 2026 sur un moniteur 1080p en **HDMI direct**, configuration parfaitement banale — le compositeur re-présente la dalle entière à chaque frame et les `dirtyRects` valent en permanence un rect unique pleine taille (`1 rect (0, 0) 1920×1080`, 643 frames sur 644, Bureau nu), alors que les pixels ne changent pas : deux captures à une seconde d'écart diffèrent de **0,03 %**. Le phénomène est connu des Mac Apple Silicon sur HDMI (tramage temporel, pipeline YCbCr) ; une première hypothèse — transport virtualisé type dock DisplayLink — a été **réfutée** par cette même mesure, l'écran étant branché en direct. Un papier peint vidéo produit le même symptôme, pour une cause réelle cette fois.

Sur ces écrans, tout ce qui se nourrit des `dirtyRects` déraille en chaîne : le critère de surface est aveugle (burst à chaque marque), l'encodeur ne s'arrête jamais, et les budgets CPU/disque du § 4.4 sont crevés par du contenu immobile — 7,95 Mb/s mesurés pour un Bureau figé. Le remède est l'**épreuve d'identité par échantillonnage épars** (§ 5.1 bis) : quelques milliers de pixels comparés à la frame précédente démasquent la frame déclarée sale mais identique, pour un coût sans rapport avec la comparaison intégrale que le § 5.1 exclut. La ligne `dernière frame` du bilan de flux rend le diagnostic lisible en une ligne.

**Limite assumée du critère de surface** : les `dirtyRects` rapportent ce que le compositeur considère changé, pas ce qui a changé en pixels. Une application à rendu GPU re-soumet sa surface entière au moindre changement — Warp à chaque battement de curseur (0,981 de surface moyenne mesurée le 23 août 2026 sur un terminal immobile), et **Terminal.app sur macOS 26 fait de même** (0,954 pour un spinner d'un seul caractère). Sur ces applications — c'est-à-dire l'essentiel des applications modernes — l'axe surface du critère mesure de fait la *fraction d'écran de la fenêtre qui change*, pas la surface des pixels changés. Le critère y reste utile sous cette lecture : une petite fenêtre qui s'anime ne déclenche pas le burst, une grande le déclenche — et une grande fenêtre re-soumise à haute fréquence coûte trois images au lieu d'une, même pour des pixels identiques. Distinguer une surface re-soumise d'une vraie animation exigerait de comparer les pixels, ce que le § 5.1 exclut par coût (440 MiB/s). Le surcoût est borné et assumé.
